...
(function($, window, document, undefined) {
    ...
    var defaults = {
            ...
            checkOverlaySize: 16,
            //利用率覆盖图标
            rateOverlayImage: [{ src: "low.png", hsrc: "low-h.png", min: 0, max: 30 }, { src: "middle.png", hsrc: "middle-h.png", min: 30, max: 60 }, { src: "high.png", hsrc: "high-h.png", min: 60 }],
            //利用率覆盖图标大小
            rateOverlaySize: 6,
            //箭头移动的步长
            moveStepSize: 2,
            ...
            cabInfoAction: "",
            //机柜信息和设备列表假数据
            fakeCabInfo: { "deviceList": [{ "SmallType": "OptiXOSN1800V", "IP": "129.8.10.87", "CabinetPosition": "A1-1", "TotalRate": 20 }, { "SmallType": "OptiXOSN1800U", "IP": "129.8.9.1", "CabinetPosition": "A1-2", "TotalRate": 70 }], "uNum": [{ "no": "A1", "usedU": 11, "allU": 80 }] },
            //获取机柜U位数
            cabUCountAction: "",
            //机柜U位数假数据
            fakeCabUCount: [{ "no": "A1", "usedU": 11, "allU": 80 }, { "no": "A2", "usedU": 19, "allU": 80 }, { "no": "A3", "usedU": 22, "allU": 80 }, { "no": "A4", "usedU": 28, "allU": 80 }, { "no": "A5", "usedU": 0, "allU": 80 }, { "no": "A6", "usedU": 0, "allU": 80 }, { "no": "A7", "usedU": 36, "allU": 80 }, { "no": "A8", "usedU": 36, "allU": 80 }, { "no": "A9", "usedU": 34, "allU": 80 }, { "no": "A10", "usedU": 69, "allU": 80 }, { "no": "A11", "usedU": 69, "allU": 80 }, { "no": "A12", "usedU": 45, "allU": 80 }, { "no": "A13", "usedU": 69, "allU": 80 }, { "no": "A14", "usedU": 51, "allU": 80 }],
            //更新已用机柜数
        ...
        //模板
        templates = {
            tooltip: '<div class="{cls}" data-name="{target}"><div class="title"><span>{name}</span><a href="javascript:void(0)">查看详情</a></div>' +
                ...
                '<table class="devices"><thead><tr><th>设备({device_count})</th><th>IP</th><th>设备利用率</th><th>位置</th></tr></thead><tbody>{device_html}</tbody></table></div>',
        ...
        data = {
            //展示模式的工具栏
            tools: [{ type: "zoomIn", onclick: function() { this.zoomIn(); } },
                { type: "zoomOut", onclick: function() { this.zoomOut(); } },
                { type: "actualSize", onclick: function() { this.actualSize(); } },
                { type: "download", onclick: function() { this.exportImage(); } },
                { type: "print", onclick: function() { this.print(); } }
            ],
            //展示模式的提示栏
            tip: [{ cls: "low", text: "0-30" },
                { cls: "middle", text: "30-60" },
                { cls: "high", text: "60-100" }
            ],
            //编辑模式的工具栏
            etools: [{ type: "zoomIn", onclick: function() { this.zoomIn(); } },
                { type: "zoomOut", onclick: function() { this.zoomOut(); } },
                { type: "actualSize", onclick: function() { this.actualSize(); } },
                { type: "separator" },
                { type: "undo", onclick: function() { this.undo(); }, key: "Ctrl+Z" },
                { type: "redo", onclick: function() { this.redo(); }, key: "Ctrl+Y" },
                { type: "separator" },
                { type: "selectAll", onclick: function() { this.selectAll(); }, key: "Ctrl+A" },
                { type: "del", onclick: function() { this.remove(); }, key: "Delete" },
                { type: "clear", onclick: function() { this.clear(); }, key: "Ctrl+Delete" },
                { type: "separator" },
                { type: "restore", onclick: function() { this.restore(); } },
                { type: "save", onclick: function() { this.save(); }, key: "Ctrl+S" }
            ],
            //编辑模式的选中右键菜单
            emenus4cell: [{ type: "copy", onclick: function() { this.copy(); }, key: "Ctrl+C" },
                { type: "del", onclick: function() { this.remove(); }, key: "Delete" }
            ],
            //编辑模式的右键菜单
            emenus: [{ type: "paste", onclick: function() { this.pasteHere(); }, key: "Ctrl+V" },
                { type: "selectAll", onclick: function() { this.selectAll(); }, key: "Ctrl+A" }
            ],
            //编辑模式的键盘快捷键
            shortcuts: [{ keyCode: 46, key: "Delete", action: function() { this.remove(); } },
                { keyCode: 46, key: "Ctrl+Delete", isCtrl: true, action: function() { this.clear(); } },
                { keyCode: 65, key: "Ctrl+A", isCtrl: true, action: function() { this.selectAll(); } },
                { keyCode: 90, key: "Ctrl+Z", isCtrl: true, action: function() { this.undo(); } },
                { keyCode: 89, key: "Ctrl+Y", isCtrl: true, action: function() { this.redo(); } },
                { keyCode: 67, key: "Ctrl+C", isCtrl: true, action: function() { this.copy(); } },
                { keyCode: 86, key: "Ctrl+V", isCtrl: true, action: function() { this.paste(); } },
                { keyCode: 83, key: "Ctrl+S", isCtrl: true, action: function() { this.save(); } }
            ]
        },
        //辅助方法
        utils = {
        ...
        plugin = {
            ...
            save: function(instance) {
                ...
                $.post(instance.options.saveAction, {
                    ...
                    data: JSON.stringify({ vertex: labData })
                }, function(data) {
            ...
            getUsedCells: function(instance, vertexs) {
                return $.grep(vertexs || instance.graph.getChildVertices(), function(n) { return n.value && n.value.name });
            },
            ...
            setToggleIcon: function(instance, $box, hideCls, showCls) {
                var self = this,
                    $toggle = $(utils.tpl(templates.toggle_icon, { cls: hideCls, text: self.getText(instance, "hide") })).insertAfter($box);
                ...
                graph.getTooltipForCell = typeof tooltip === "function" ? function(cell) {
                    ...
                    self.getCabInfo(instance, value.name, cell.style).done(function(data) {
                        var cabInfo = instance.cabInfo,
                            ...
                            cabInfo = cabInfo.uNum[0] || { allU: 0, usedU: 0 },
                            device_html = "";
            ...
            setToolbar: function(instance) {
                var $div = $('<div class="' + classList.toolbar + '"></div>').prependTo($(instance.dom)),
                    ...
                    $tip = $(utils.tpl('<ul class="{cls}"><li>{text}(%)</li></ul>', { cls: classList.tip, text: this.getText(instance, "capacity") })).appendTo($div),
                    self = this;
            ...
            initCabPropbox: function(instance, cell) {
                ...
                if ($div.hasClass("loaded")) {
                    ...
                    $div.find("." + classList.ePropboxCabList + ">li").
                    removeClass(selectedCls).filter('[data-name="' + groupName + '"]').
                    find("." + classList.ePropboxCabTitle).trigger("click");
                    $div.find("." + classList.ePropboxCabItem + ">li").
                    removeClass(selectedCls).filter('[data-name="' + name + '"]').
            ...
            getCabInfo: function(instance, cabNo, cabType) {
                var opts = instance.options,
                    ...
                    IsSpcCab = cabType === "cab-dev" || cabType === "cab-iond" || cabType === "cab-server"; //是否是特殊机柜（U位占用率为百分百）
                if (!url) {
            ...
            exportImage: function(instance) {
                ...
                new mxXmlRequest(url, 'filename=' + lab + '.svg&format=svg' +
            ...
            overridePrototype: function() {
                ...
                mxTooltipHandler.prototype.show = function(tip, x, y) {
                    if (!this.destroyed && tip != null && tip.length > 0) {
                        ...
                        this.div.style.top = (Math.min(y, 580) + mxConstants.TOOLTIP_VERTICAL_OFFSET +
                            origin.y) + 'px';
