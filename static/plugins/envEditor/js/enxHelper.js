/*
   说明：此文档主要用于Enx和列表之间的互转。ENX->JSON列表  ，JSON列表转ENX文件
   注意点：Enx在直接jquery对象化XML文档时，会改变属性的顺序（解析时，固定顺序对照表），并且在插入属性时会自动变换属性位置（字符串连接）；
*/
...
var EnxHelper = new function () {
    ...
    this.PreDealEnxContent = function (strEnxContent) {
        return strEnxContent.replace(new RegExp(this.GlobalVar.XMLTAG, "gm"), "").replace(new RegExp(EnxHelper.GlobalVar.IllegalTagEnd, "gm"), EnxHelper.GlobalVar.LawfulTagEnd)
            .replace(new RegExp(EnxHelper.GlobalVar.LinkTag, "gm"), EnxHelper.GlobalVar.LinkTagReplace);
    };
    ...
    this.EnxToLstJson = function (strEnxContent) {
        ...
        var rootNodeJson = {};
        rootNodeJson[EnxConstField.RootNodeAttr] = EnxHelper.GetNodeAttr(rootNode);
        //一层子节点
    ...
    this.LstJsonToEnx = function (lstJson) {
        ...
        //01 寻找环境节点；
        var rootNode = JsonHelper.GetJsonByKey(lstJson, EnxHelper.Field.BigType, EnxHelper.ModelType.Env);

        //如果为空，创建一个环境属性值
        if (!JsonHelper.IsJsonObj(rootNode) || JsonHelper.IsEmptyJson(rootNode) || !rootNode) {
            rootNode = EnxHelper.ParameterNodeToJson(null);
        }
        ...
        rootParametersContent[0].innerText = EnxHelper.XmlGetParameter(rootNode, true);
        ...
        //02.04 反向解析toplimit
        var topLimitNode = JsonHelper.GetJsonByKey(lstJson, EnxHelper.Field.BigType, EnxHelper.ModelType.Topolimit);
        var topLimitContent = $(EnxConstField.XMLTAG.TOPOLIMIT);
        topLimitContent[0].innerText = EnxHelper.Format(EnxHelper.GlobalVar.FormatTpl.CDATA, topLimitNode[EnxHelper.Field.Name] || "");
    ...
    this.XmlGetParameter = function (json, isReverse) {
        if (this.GlobalVar.UserDefineField.length == 0) {
            this.GlobalVar.UserDefineField = JsonHelper.GetJsonAllValue(EnxHelper.Field);
        ...
        for (var key in json) {
            if (this.GlobalVar.UserDefineField.indexOf(key) < 0) {
                ...
                var str = EnxHelper.Format(EnxHelper.GlobalVar.FormatTpl.Paratemter, attrStr);
    ...
    this.XMLJsonToNode = function (parentJson, parentNode, lstJson) {
        var parentId = JsonHelper.GetStrValue(parentJson, EnxHelper.Field.ID);
        //获取子设备节点
        var childNodes = [];
        ...
        if (childNodes && childNodes.length > 0) {
            ...
            $.each(childNodes, function (index, childNode) {
                ...
                var parameter = EnxHelper.XmlGetParameter(childNode);
                parametersConten[0].innerText = parameter;
                deviceContent.append(parametersConten);
                EnxHelper.XMLJsonToNode(childNode, deviceContent, lstJson);
    ...
    this.LinkNodeToJson = function (linkNode) {
        var result = {};
        var paramNodes = EnxHelper.GetChildNode(linkNode);
        $.each(paramNodes, function (index, paramNode) {
            var fieldName = $(paramNode).attr(EnxConstField.AttrName.ENXFILE_ATTR_NAME);
            if (fieldName) {
                result[EnxHelper.Field.BigType] = EnxHelper.ModelType.Link;
                result[fieldName] = EnxHelper.GetNodeAttr(paramNode);
            }
        });
    ...
    this.NodeAttachFieldId = function (nodeJson) {
        ...
        var idValue = guidValue ? guidValue : EnxHelper.CreateNewGuid();
        nodeJson[EnxHelper.Field.ID] = idValue;
    }

    //解析参数节点
    this.ParameterNodeToJson = function (paramNode) {
        var result = {};
        result[EnxHelper.Field.BigType] = EnxHelper.ModelType.Env;
        var paramNodes = EnxHelper.GetChildNode(paramNode);
        $.each(paramNodes, function (index, tempParamNode) {
            var fieldName = $(tempParamNode).attr(EnxConstField.AttrName.ENXFILE_ATTR_NAME);
            //获取参数节点的所有属性
            result[fieldName] = EnxHelper.GetNodeAttr(tempParamNode);
        });
        this.NodeAttachFieldId(result);
        return result;
    };
    ...
    this.DeviceNodeToJson = function (deviceNode, parentNodeJson, isTopDevice) {
        var result = [];
        var curDevice = {};
        curDevice[EnxHelper.Field.ParentGID] = parentNodeJson[EnxHelper.Field.ID] || "";
        curDevice[EnxHelper.Field.IsTopDevice] = isTopDevice ? EnxConstField.BoolStr.Yes : EnxConstField.BoolStr.No;
        var childrenDevice = [];
        var childrenNodes = EnxHelper.GetChildNode(deviceNode);
        ...
        if (curDevice.objname) {
            this.ResetMaxIndex(curDevice[EnxHelper.Field.BigType], curDevice.objname.value);
    ...
    this.Format = function (source, params) {
        if (arguments.length == 1)
            return function () {
                ...
                args.unshift(source);
                return EnxHelper.Format.apply(this, args);
            };
    ...
    this.GetChildNode = function (node) {
        ...
        if (node && node.childNodes) {
            $.each(node.childNodes, function (index, childNode) {
                if (childNode.nodeName != EnxHelper.GlobalVar.ExceptionNode) {
...
var JsonHelper = new function () {
        ...
        if (arrJson && arrJson.length > 0) {
            for (var i = 0; i < arrJson.length; i++) {
                ...
                if (this.IsContainKey(json, Item[nameKey])) {
                    ...
                    Item[valueKey] = enumType["default"];  //排序必须有个默认值
                }
            }
        }
        return resJson;
    ...
    this.ParseArrStrToArrJson = function (arrStr) {
        ...
        if (arrStr && arrStr.length > 0)
            $(arrStr).each(function (index, value) {
    ...
    this.GetListValueByKey = function (arrJson, key) {
        var arrField = [];
        if (arrJson && arrJson.length > 0)
            $(arrJson).each(function (index, value) {
        ...
        if (arrJson && arrJson.length > 0)
            for (var i = 0; i < arrJson.length; i++) {
                ...
                if (keyValue == jsonValue) {
                    ...
                    for (var tempKey in newJson) {
                        if (!jsonItem[tempKey]) {
                            jsonItem[tempKey] = { name: tempKey, value: newJson[tempKey] };
                        }
