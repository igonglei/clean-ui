    ...
    this.BindEvent = function (plugObj) {
        ...
        $(".spanEditorBtnOpenEnx").off("click").click(function () {
            ...
            $(".EPlugImportSubmit").off("click").click(function () {
                ...
                plugObj.Property.FileName = fileName.substr(fileName.lastIndexOf("\\") + 1); //文件名称
...
function EPlug() {
    ...
    this.Property = new function () {
        ...
        //组网的id
        this.EnxId = EnxHelper.CreateNewGuid(); // + this.FileAfter;

        //Mode，默认为2,用户自己添加端口，添加连线时弹框有选择的连接哪些端口
        this.Mode = 2;
    ...
    this.Init = function (elementId, option) {
        ...
        var svcurl = EnxHelper.GetQueryString(Me.ConstInfo.QueryStringParam.svcurl);

        Me.Property.CurrentPDU = pdu ? pdu : Me.Property.DefaultPDU;
        var postHost = svcurl ? svcurl : Me.PduRelUrl[Me.Property.CurrentPDU];

        if (!postHost) {
            postHost = Me.PduRelUrl[Me.Property.DefaultPDU];
        }

        //最后一位字符不是/，加上/
        if (postHost.lastIndexOf("/") != (postHost.length - 1)) {
            postHost += "/";
        }
    ...
    this.GetEnxContentByID = function (enxId) {
        ...
        if (Me.Property.PostUrl.GetById.indexOf(Me.Property.WebsvcSuffix) == -1) {
            ...
            reqType = "POST";
            param = JSON.stringify({ "user": strUser, "enxId": enxId });
        }
        var service_addr = Me.Property.PostUrl.GetById;
        var req_content = param;
        var headerSet = JSON.stringify({ "X-Auth-UserID": strUser });
        var $content = $(".EplugGraphEditorRightContainer");
        $content.showLoader();
        this.RequestService(reqType, service_addr, req_content, headerSet,
            function (res) {
                $content.hideLoader();
                var alertMsg = "";
    ...
    this.GetEnxContent = function () {
        ...
        xmlContent = xmlContent.replace(new RegExp("&lt;", "gm"), "<")
    ...
    this.GetPostData = function () {
        ...
        if (Me.Property.PostUrl.UpdateById.indexOf(Me.Property.WebsvcSuffix) == -1) {
            ...
            objData = { "user": Me.Property.CurrentUser, "enxId": Me.Property.EnxId, "enxContent": xmlContent, "enxImageUrl": imageUrl, "fileName": fileName };
        }
        return objData;
    }
    ...
    this.SaveData = function (isAlert) {
        ...
        var headerSet = { "X-Auth-UserID": Me.Property.CurrentUser };
        //请求服务
    ...
    this.SaveDataOld = function (isAlert) {
        var objData = this.GetPostData;
        //请求数据
        $.ajax({
            type: "POST",
            url: Me.Property.PostUrl.UpdateById,
            data: JSON.stringify(objData),
            dataType: "json",
            contentType: "application/json; charset=utf-8",
            beforeSend: function (request) {
                if (Me.Property.PostUrl.UpdateById.indexOf(Me.Property.WebsvcSuffix) == -1) {
                    request.setRequestHeader("X-Auth-UserID", strUser);
                }
            },
                ...
                if (Me.Property.PostUrl.UpdateById.indexOf(Me.Property.WebsvcSuffix) == -1) {
                    ...
                    if (data && data.d) {
                        ...
                        if (!res.errcode) {
                            ...
                            if (isAlert && res.errcode) {
                                toastr.error(res.errcode);
                            }
                        }
                    }
                }
    ...
    this.AddDevice = function (deviceinfo) {
        //0.解析deviceinfo
        //1.调用Me.CreateModel(type, parentId, pareantName, parentType,id,jsonvalue)，更新Me.Property.DataList
        $.each(deviceinfo, function (index, value) {
            var parentType = "";
            var _device = value; //JSON.parse(value);
            var cur_device = {};
            cur_device[EnxConstField.AttrName.ENXFILE_ATTR_NAME] = _device.name;
            cur_device[EnxConstField.AttrName.ENXFILE_ATTR_OBJNAME] = _device.name;
            cur_device["ip"] = _device.ip;
            cur_device["type"] = _device.type;
            cur_device["username"] = _device.username;
            cur_device["password"] = _device.password;
            if (_device.virtual_real) {
                cur_device["virtual_real"] = "Real";
            } else {
                cur_device["virtual_real"] = "Virtual";
            }
            if (_device.BigType == "Port") {
                cur_device["host_id"] = _device.host_id;
                parentType = "Ne";
            }
            Me.CreateModel(_device.BigType, _device.parentid, "", parentType, _device.id, cur_device);
        });

        //2.重新加载页面 Me.SetData(Me.Property.DataList)
        Me.SetData(Me.Property.DataList);
    }
    ...
    this.HtmlEditor = new function () {
        ...
        this.InitAvailField = function (type) {
            ...
            var json = this.GetFirstModelEntity(type); //应该是取默认的
        ...
        this.GetFirstModelEntity = function (type) {
            var res = Me.ConstInfo.DefaultAvailField[type]; //默认字段{};
            return res;
        };
    ...
    this.PduRelUrl = {
        ...
        "lcm_sde": "http://lcm.huawei.com/env/rest/",
        "lcm_sdv": "http://sdv.lcm.huawei.com/env/rest/",
        "lcm_terminal": "http://lcm-device.huawei.com/env/rest/",
        "SYSTEM_TEST": "http://localhost:50038/",
        "SYSTEM_WS": "http://10.188.67.220:30000/EnxWebService.asmx/",
        //"ROUTER": "http://10.93.122.183:42514/EnxWebService.asmx/",
        "ROUTER": "http://10.162.195.147:6789/WebService1.asmx/"
    }
    ...
    //获取options
    this.GetOptionValue = function (fieldName) { }
    ...
    this.ConstInfo = new function () {
        ...
        this.DefaultAvailField = new function () {
            ...
            //Eth的有效字段
            this.Eth = this.CommonAvailField.concat(["actor", "bid", "sbid", "pid", "connect_type", "engrossstate", "groupip", "id", "mac", "username", "password", "port", "portnum",
                "ptp", "realNEId", "realtype", "submask", "vender"
            ]);
        ...
        this.SelectField = [
            ...
            "vmFlag", //配置标识 下拉选择，然后CPU内存 和镜像 改成自动显示 不可修改的 用户选择好 配置标识以后 他们两个自动显示出来

            "network", //网络平面。网卡/端口的属性

            "internal_external", //端口类型（内联端口/外联端口）

            "virtual_real" //网元类型（虚拟网元/真实网元）
        ...
        this.QueryStringParam = new function () {
            ...
            this.enxId = "enxId";
            //mode代表含义：
            //1：编辑模式1，添加连线时自动添加端口（默认）
            //2：编辑模式2，用户自己添加端口，添加连线时弹框有选择的连接哪些端口
            //3：查看模式，不能添加，没有列表数据
            //4：查看模式，看图和列表
            this.mode = "mode";
            //svcurl。用户的服务地址
...
var GraphHelper = function (edit) {
    //由于生成图片的时候，只有绝对地址才有效，所以此处获取本地域名
        ...
        $.each(selectLinks, function (index, link) {
            if (hasDealLinksId.indexOf(link.id) < 0) {
                ...
                $.each(selectLinks, function (index, tempLink) {
                    if (tempLink.id != link.id &&
                        ((tempLink.source.id == sourceDeviceId && tempLink.target.id == targetDeviceId) ||
                            (tempLink.target.id == sourceDeviceId && tempLink.source.id == targetDeviceId))) {
    ...
    this.SubGraph = new function () {
        ...
        this.Init = function () {
            ...
            SubGraphObj.Graph.isCellMovable = function (cell) {
                return oldMovable.apply(this, arguments) &&
                    currentPermission.canMoveVertet;
            };
        ...
        this.BasicFixSet = function () {
            ...
            //设置对象选中状态的颜色、线宽
            mxConstants.VERTEX_SELECTION_COLOR = '#87CEEB'; // 浅蓝色

            mxConstants.VERTEX_SELECTION_STROKEWIDTH = 1.3; // 2像素

            mxConstants.VERTEX_SELECTION_DASHED = false; //不要虚线

            mxConstants.EDGE_SELECTION_STROKEWIDTH = 1.3; // 2像素
            ...
            new mxCellTracker(this.Graph, '#87CEEB'); //鼠标停留高亮元素
        ...
        this.RemoveCellRel = function (cell) {
            ...
            } else {//如果删除是单板或者子卡时，找出所有的占有端口 [网元编辑面板]
            ...
            $.each(usedPortLst, function (index, usedPort) {
                var edges = $.grep(EnvEditor.PlugObj.Property.LstLinkJson, function (n, m) { return n.sourcedeviceid.value == usedPort.id || n.targetdeviceid.value == usedPort.id });
        ...
        this.InitLinkEditor = function (link) {
            ...
            var portTypeShow; //显示哪种类型的端口（1内联口，2外联口，空全部）
                    ...
                    $.each(links, function (index, link) {
                        ...
                        if (data) {
                            ...
                            if (source && target) {
                                ...
                                var tempGraphLink = SubGraphObj.Graph.insertEdge(parentNode, link.id, link.objname, portDic[sourceId], portDic[targetId], EnxHelper.ModelType.Link); //EnxHelper.ModelType.Link
            ...
            $.each(childNodeJson, function (index, jsonNode) {
                ...
                if (isPortConnect && jsonNode.BigType == EnxHelper.ModelType.Port && jsonNode.internal_external && jsonNode.internal_external.value) {
                    ...
                    if (portTypeShow == 1 && internal_external_value > -1) {
                        return true; //要显示内联口，外联口不显示。当前端口是外联口(internal_external_value > -1)时break
                    }
                    if (portTypeShow == 2 && internal_external_value == -1) {
                        return true; //要显示外联口，内联口不显示。当前端口是内联口(internal_external_value == -1)时break
                    }
                }

                //连线面板，如果不是两个不同源段连线，网元内部的网络平面也不显示（网元内部的网络平面只连接网元内部的端口）
                //if (isPortConnect && jsonNode.BigType == EnxHelper.ModelType.Network && portTypeShow == 2) {
                //    return true;
                //}
    ...
    this.PropertyPanel = new function () {
        ...
        this.HtmlTemplate = new function () {
            ...
            this.FieldEditItem = '<tr data-fieldname="{0}">\
                                      <td class="EplugGraphEditorPropertyPanelFieldTitle EPlugTextEllipsis">{1}:</td>\
                                      <td><input value="{2}" type="text" class="field_value" /></td>\
                                   </tr>';

            //模板 {0} 字段名称  {1}字段的显示值  {2}真实值  {3}显示的值
            this.FieldShowItem = '<tr data-fieldname="{0}">\
                                      <td class="EplugGraphEditorPropertyPanelFieldTitle EPlugTextEllipsis">{1}:</td>\
                                      <td><label class="EPlugTextEllipsis EplugGraphEditorPropertyPanelFieldLink" title="{3}" data-val="{2}">{3}</label></td>\
                                   </tr>';

            //模板 {0} 字段名称  {1}字段的显示值 {2}真实值
            this.FieldEditSelItem = '<tr data-fieldname="{0}">\
                                      <td class="EplugGraphEditorPropertyPanelFieldTitle EPlugTextEllipsis">{1}:</td>\
                                      <td><select>{2}</select></td>\
                                   </tr>';
            //select的option模板
            this.SelectOptionItem = '<option value="{0}" {2}>{1}</option>';

            this.FieldEditCustomItem = '<tr class="custom_field" data-fieldname="{0}">\
                                      <td><input type="text" class="custom_field_key" placeholder="Key" value="{0}" /><span>:</span></td>\
                                      <td><input type="text" class="custom_field_value field_value" placeholder="Value" value="{1}" /><span class="glyphicon glyphicon-plus custom_field_add" aria-hidden="true" title="Add"></span><span class="glyphicon glyphicon-minus custom_field_delete" aria-hidden="true" title="Delete"></span></td>\
                                   </tr>';
        }
        ...
        this.updateProperties = function () {
            ...
            var updatedJson = JsonHelper.JsonArrSingleUpdate(edit.Property.DataList, EnxHelper.Field.ID, EnxConstField.AttrName.ENXFILE_ATTR_VALUE,
                PropertyPanelObj.EditNodeJson._id, newNodeJson);
        ...
        this.EditNodeById = function (id) {
            ...
            $.each(fieldLst, function (index, fieldName) {
                ...
                if (edit.ConstInfo.NoEditField.indexOf(fieldName) < 0) {
                    if (specialPortField.indexOf(fieldName) > -1) {
                        ...
                        var options = ""; //EnxHelper.Format(PropertyPanelObj.HtmlTemplate.SelectOptionItem, "", "", "");
                        var selectOptionValues = edit.Property.SelectOptionValues[fieldName];
            ...
            var cusKeys = $.grep(allkeys, function (n) { return $.inArray(n, nokeys) == -1; });
    ...
    //获取对端节点
    this.GetPeerCell = function (cell, containLoop) {
        var result = [];
        var links = cell.edges;
        if (links && links.length > 0) {
            $.each(links, function (index, link) {
                if (cell.id == link.source.id && cell.id == link.target.id) {
                    ...
                    if (cell.id == link.source.id) {
                        result.push(link.target);
                    }
                    if (cell.id == link.target.id) {
    ...
    this.GetDisplayNodeName = function (nodeJson, isPortConnect) {
        ...
        if (isPortConnect && nodeJson.BigType == EnxHelper.ModelType.Port) {
            ...
            if (interface_value) {
                //displayNodeName += "(" + interface_value + ")";//显示不全，只显示interface
    ...
    this.BasicFixSet = function () {
        ...
        mxConstants.DEFAULT_FONTFAMILY = 'Microsoft YaHei'; //字体
        ...
        mxConstants.EDGE_SELECTION_COLOR = '#87CEEB'; // 浅蓝色

        mxConstants.EDGE_SELECTION_DASHED = false; //不要虚线

        mxConstants.EDGE_SELECTION_STROKEWIDTH = 1.3; // 2像素

        mxConstants.DEFAULT_FONTSIZE = 14; // 字体大小
        ...
        mxEvent.disableContextMenu(this.Property.Contain); //禁用鼠标右键菜单

        new mxCellTracker(this.Property.CoreGraph, '#87CEEB'); //鼠标停留高亮元素
    ...
    this.ExtendsHook = function (graph) {
        ...
        graph.isCellDisconnectable = function (cell, terminal, source) {
            return oldDisconnectable.apply(this, arguments) &&
                currentCorePermission.editEdges;
        };
    }
    ...
    //路由器定制获取比较后的最大索引 比如有某个网元ne1和ne2 有根快速连线，  但是ne1端口1名称改了，但是端口命名还只能在2开始
    this.GetMaxComparePortIndex = function (source, target) {
        var sourcePortMaxIndex = this.GetMaxSinglePortIndex(source, target);
        var targetPortMaxIndex = this.GetMaxSinglePortIndex(target, source);
        var resIndex = sourcePortMaxIndex > targetPortMaxIndex ? sourcePortMaxIndex : targetPortMaxIndex;
        return resIndex;
    }

    //路由器定制  指向性端口名称
    this.GetPortPointName = function (source, target) {
        var preName = source.objname + EnxConstField.NameSeparate + EnxConstField.NamePoint + EnxConstField.NameSeparate + target.objname + EnxConstField.NameSeparate;
        return preName;
    }

    //路由器定制 根据网元获取其下的最大端口索引 ne1_to_ne2_1
    this.GetMaxSinglePortIndex = function (source, target) {
        var childrens = edit.GetChildrens(source.id, true);
        var preName = this.GetPortPointName(source, target);
        var resIndex = 1;
        $.each(childrens, function (index, value) {
            var childName = value.objname.value;
            if (value.BigType == EnxHelper.ModelType.Port && childName.indexOf(preName) == 0) {
                var portIndex = parseInt(childName.replace(preName, ""));
                resIndex = portIndex > resIndex ? portIndex : resIndex + 1;
            }
        });
        return resIndex;
    }
    ...
    this.GetTopDeviceBond = function (type, x, y) {
        var bond = [x, y, 62, 62]; //图片统一80x78尺寸
        return bond;
    }
    ...
    this.DropInElement = function (graph, evt, target, x, y, sourceElem) {
        ...
        if (thisObj.EditType == thisObj.ConstInfo.EnumEditType.Element) {
            ...
            if (targetType == EnxHelper.ModelType.Ne && (modelType == EnxHelper.ModelType.Board || modelType == EnxHelper.ModelType.Port || modelType == EnxHelper.ModelType.VM || modelType == EnxHelper.ModelType.Network)) {
                parent = target;
            } else if (((targetType == EnxHelper.ModelType.Board || targetType == EnxHelper.ModelType.VM) && (modelType == EnxHelper.ModelType.SubBoard || modelType == EnxHelper.ModelType.Port)) ||
                (targetType == EnxHelper.ModelType.SubBoard && modelType == EnxHelper.ModelType.Port)) {
                parent = target;
        ...
        thisObj.Property.CoreGraph.popupMenuHandler.factoryMethod = function (menu, cell, evt) {
            ...
            if (cell && cell.edge) {
                ...
                var sameDoublePoint = $.grep(selectItems, function (selectCell, selectIndex) {
                    return selectCell.edge && selectCell.id != cell.id &&
                        ((selectCell.source.id == sourceDeviceId && selectCell.target.id == targetDeviceId) ||
                            (selectCell.target.id == sourceDeviceId && selectCell.source.id == targetDeviceId));
                });
    ...
    //编辑模式，主要是适配路由器连线后直接添加端口；
    this.EditMode = this.ConstInfo.EnumEditMode.Quick;
