...
(function($, window, undefined) {
    ...
    plugin = {
        ...
        init: function(instance) {
            var self = this, $el = $(instance.dom), opts = instance.options, divs = opts.divs, margin = opts.margin, divHeight = $el.height() / divs - margin;
        ...
        fillColor: function(instance, $div, opts) {
            var self = this, divs = opts.divs, colorDivs = Math.ceil(opts.value * divs), colors = this.getColor(opts.color, divs), animation = opts.animation, duration = opts.duration, fill = function(index) {
        ...
        getColor: function(color, divs) {
            ...
            $.each(color, function(i, v) {
                var arr = v.split(","), bgColor = arr[0], borderColor = arr[1], range = Math.round(self.parsePercent(arr[2]) * divs);
        ...
        beat: function(instance, $div, colorDivs, colors) {
            var opts = instance.options, beat = opts.beat;
            if (beat <= 0) {
                return;
            }
            var divs = opts.divs, start = Math.max(0, colorDivs - beat - 1), end = Math.min(divs, colorDivs + beat), i = colorDivs, asc = true, defaultColor = {
                "background-color": "",
                "border-color": opts.borderColor
            };
